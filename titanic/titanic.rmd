...
t <- as.data.frame(table(wanted(titanic$Survived)))
colnames(t) <- c("Status","Freq")
print(t)

ggplot(titanic, aes(x=wanted(Survived),)) +
...
```{r echo=False}
# dmy vars Sex
# numbers <- sapply(titanic,class) %in% c("integer","numeric")
