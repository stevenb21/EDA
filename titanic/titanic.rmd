...
We have `r sum(is.na(titanic))` total missing values.

Let's look at the correlation matrix to figure out which features stick out.
...
It appears Sex and PClass are the best features to use to explain who survived. Let's look at those first.
...
```{r echo=FALSE}
#Scatterplot of fare/histogram with bins
# correlation/dependance on fare. is fare redundant with Pclass?

```


```{r echo=FALSE}
# Embarked count w/ survival fill | sex fill | Pclass fill
```
